...
async function pruneOldJobs() {
  ...
  const result = await db.execute({
    sql:  `DELETE FROM jobs WHERE scraped_at < $cutoff`,
    args: { cutoff },
  })
...
async function getJobs(filters = {}) {
  ...
  const result = await db.execute({
    sql:  `SELECT * FROM jobs ${where} ORDER BY scraped_at DESC LIMIT 500`,
    args,
  })
...
module.exports = { db, initDb, upsertJob, upsertJobs, pruneOldJobs, getJobs }